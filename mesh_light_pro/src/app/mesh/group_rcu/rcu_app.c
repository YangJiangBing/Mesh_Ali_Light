...
*********************************************************************************************************
* @file      rcu_app.c
* @brief    mesh RCU application file
* @details
* @author    elliot chen
* @date      2018-9-13
* @version   v1.0
...
void app_handle_io_msg(T_IO_MSG io_msg)
{
    ...
    case IO_MSG_TYPE_KEYSCAN:
        {
            if (key_handle_global_data.combine_keys_status != INVALID_COMBINE_KEYS_BIT_MASK)
            {
                ...
                if (combine_keys_detection_timer != NULL)
                {
            ...
            {
                key_handle_release_event(io_msg.u.buf);
            }
    ...
    case IO_MSG_TYPE_TIMER:
        if (io_msg.subtype == IO_MSG_KEYSCAN_LONG_PRESS)
        {
            key_handle_long_pressed_event(io_msg.u.buf);
        }
        else if (io_msg.subtype == IO_MSG_KEYSCAN_REPEAT_SEND)
        {
            key_handle_pressed_event(io_msg.u.buf);
        }
        break;
...
void app_handle_dev_state_evt(T_GAP_DEV_STATE new_state, uint16_t cause)
{
    ...
    if (gap_dev_state.gap_init_state != new_state.gap_init_state)
    {
        if (new_state.gap_init_state == GAP_INIT_STATE_STACK_READY)
        {
            ...
            /* Enable peripheral NVIC after GAP stack ready */
            app_nvic_config();
            /* Initialize group protocol transmission */
            group_transmitter_init();
        }
    }
...
T_APP_RESULT app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
    ...
    else if (service_id == dfu_server_id)
    {
        ...
        case DFU_CB_START:
            ...
            break;
        case DFU_CB_FAIL:
            LED_BLINK(LED_1, LED_TYPE_BLINK_OTA_FAIL, 5);
            break;
        case DFU_CB_END:
            LED_OFF(LED_1);
            break;
