...
void dfu_client_handle_connect(uint8_t conn_id)
{
    ...
    le_get_conn_param(GAP_PARAM_CONN_BD_ADDR, bt_addr, conn_id);
    if (0 != memcmp(bt_addr, dfu_client_ctx.bt_addr, 6))
    {
        goto end;
    }

    le_get_conn_param(GAP_PARAM_CONN_BD_ADDR_TYPE, &addr_type, conn_id);
    ...
    dfu_fsm_auto_connect = DFU_FSM_AUTO_CONN_CONNECT;
}
        ...
        case GAP_ADTYPE_MANUFACTURER_SPECIFIC:
            {
                if (length == 1 + MEMBER_OFFSET(manufacture_adv_data_t, data) + sizeof(manufacture_adv_data_dfu_t))
                {
                    ...
                    {
                        printi("dfu_client_handle_adv_pkt: product = %d-%d app = 0x%04x-0x%04x patch = %d-%d",
                                                                            ...
                                                                            dfu_ctx.patch_version > pdata->dfu.patch_version))
                        {
                            ...
                            le_connect(GAP_PHYS_CONN_INIT_1M_BIT, p_le_scan_info->bd_addr, p_le_scan_info->remote_addr_type,
