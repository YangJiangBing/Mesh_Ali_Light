...
typedef struct
{
    ...
    int32_t delta_level;
    bool max_move;
} generic_level_info_t, *generic_level_info_p;
                                               ...
                                               generic_transition_time_t remaining_time)
{
    ...
    generic_level_info_p plevel_info = pmodel_info->pargs;
                                     ...
                                     int16_t target_level, generic_transition_time_t trans_time)
{
    ...
    {
        generic_level_publish(pmodel_info, level_after_set);
    }
...
static int32_t generic_level_delay_execution(mesh_model_info_t *pmodel_info, uint32_t delay_type)
{
    ...
    case DELAY_EXECUTION_TYPE_LEVEL:
        generic_level_process(pmodel_info, GENERIC_LEVEL_SERVER_SET, 0,
                              plevel_info->target_level, plevel_info->trans_time);

        break;
    case DELAY_EXECUTION_TYPE_LEVEL_MOVE:
        generic_level_process(pmodel_info, GENERIC_LEVEL_SERVER_SET_MOVE,
                              plevel_info->move_delta, plevel_info->target_level,
                              plevel_info->trans_time);
        break;
    default:
        break;
    }
...
bool generic_level_server_reg(uint8_t element_index, mesh_model_info_p pmodel_info)
{
    ...
    {
        generic_level_info_p plevel_info = plt_malloc(sizeof(generic_level_info_t), RAM_TYPE_DATA_ON);
        if (NULL == plevel_info)
        {
