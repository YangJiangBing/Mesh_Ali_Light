                                       ...
                                       uint16_t app_key_index, time_zone_t time_zone)
{
    ...
    *((time_zone_t *) & (msg.time_zone_offset_current)) = time_zone;
...
mesh_msg_send_cause_t tai_utc_delta_status(mesh_model_info_p pmodel_info, uint16_t dst,
                                           uint16_t app_key_index, tai_utc_delta_t time_tai_utc_delta)
{
                                   ...
                                   tai_time_t time)
{
    ...
    {
        ret = time_status(pmodel_info, 0, 0, time);
    }
...
static tai_time_t get_present_time(const mesh_model_info_p pmodel_info)
{
    ...
    memset(&get_data, 0, sizeof(time_get_t));
...
static bool time_server_receive(mesh_msg_p pmesh_msg)
{
    ...
    case MESH_MSG_TIME_STATUS:
        if (pmesh_msg->msg_len == sizeof(time_status_t))
        {
            ...
            {
                /* publish time */
                time_publish(pmodel_info, *((tai_time_t *)(pmsg->tai_seconds)));
            }
        }
        break;
...
{
    time_status(pmodel_info, 0, 0, get_present_time(pmodel_info));
    return 0;
}
