...
user_cmd_parse_result_t user_cmd_list(user_cmd_parse_value_t *pparse_value)
{
    ...
    while (pelement != NULL)
    {
        ...
        while (pmodel != NULL)
        {
            ...
            if (MESH_NOT_UNASSIGNED_ADDR(pmodel->pub_addr))
            {
                data_uart_debug("-(pub:0x%04x-%d-%d)", pmodel->pub_addr, pmodel->pub_ttl,
                                pmodel->pub_key_info.app_key_index);
            }
...
user_cmd_parse_result_t user_cmd_tp_seg_start(user_cmd_parse_value_t *pparse_value)
{
    ...
    if (MESH_MSG_SEND_CAUSE_SUCCESS == tp_seg_start(dst, ttl, app_key_index, count))
    {
        return USER_CMD_RESULT_OK;
    }
    else
...
{
    log_module_trace_set(MODULE_LOWERSTACK, LEVEL_ERROR, pparse_value->dw_parameter[0] & 0x1);
    log_module_trace_set(MODULE_LOWERSTACK, LEVEL_WARN, pparse_value->dw_parameter[0] & 0x1);
    log_module_trace_set(MODULE_LOWERSTACK, LEVEL_INFO, pparse_value->dw_parameter[0] & 0x1);
    log_module_trace_set(MODULE_LOWERSTACK, LEVEL_TRACE, pparse_value->dw_parameter[0] & 0x1);

    log_module_bitmap_trace_set(0x7FFFFFFFFFFFFFFF, LEVEL_TRACE, pparse_value->dw_parameter[0] & 0x2);
    log_module_bitmap_trace_set(0x7FFFFFFFFFFFFFFF, LEVEL_INFO, pparse_value->dw_parameter[0] & 0x2);
