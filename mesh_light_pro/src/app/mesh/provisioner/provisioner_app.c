...
T_APP_RESULT app_client_callback(T_CLIENT_ID client_id, uint8_t conn_id, void *p_data)
{
    ...
    else if (client_id == datatrans_client_id)
    {
        datatrans_client_cb_data_t *pdata = p_data;
        switch (pdata->cb_type)
        {
        case DATATRANS_CLIENT_CB_TYPE_DISC_STATE:
            switch (pdata->cb_content.disc_state)
            {
            case DATATRANS_DISC_DONE:
                /** discovery data transmission service procedure successfully done. */
                data_uart_debug("datatrans service discovery end!\r\n");
                break;
            case DATATRANS_DISC_FAIL:
                /** discovery request failed. */
                data_uart_debug("datatrans service discovery fail!\r\n");
                break;
            case DATATRANS_DISC_NOT_FOUND:
                /* discovery request failed. */
                data_uart_debug("datatrans service not found!\r\n");
                break;
            default:
                break;
            }
            break;
        case DATATRANS_CLIENT_CB_TYPE_READ_RESULT:
            switch (pdata->cb_content.read_result.type)
            {
            case DATATRANS_READ_DATA_OUT_CCCD:
                if (0 == pdata->cb_content.read_result.cause)
                {
                    data_uart_debug("datatrans data out cccd = %d.\r\n",
                                    pdata->cb_content.read_result.data.datatrans_data_out_cccd);
                }
                break;
            case DATATRANS_READ_DATA_OUT:
                if (0 == pdata->cb_content.read_result.cause)
                {
                    data_uart_debug("datatrans data out = ");
                    data_uart_dump(pdata->cb_content.read_result.data.value.datatrans_data_out,
                                   pdata->cb_content.read_result.data.value.len);
                }
                break;
            default:
                break;
            ...
            break;
        case DATATRANS_CLIENT_CB_TYPE_NOTIF_IND_RESULT:
            data_uart_debug("datatrans notify = ");
            data_uart_dump(pdata->cb_content.notify_indicate_data.value.datatrans_data_out,
                           pdata->cb_content.notify_indicate_data.value.len);
            break;
        case PROV_CLIENT_CB_TYPE_WRITE_RESULT:
            break;
        default:
