                                  ...
                                  const TickType_t xOptionalValue)
{
    ...
    BaseType_t xTimerListsWereSwitched, xListWasEmpty, xResult;
    TickType_t xTimeNow;
            ...
            case tmrCOMMAND_START_DONT_TRACE:
                                               ...
                                               xMessage.u.xTimerParameters.xMessageValue) == pdTRUE)
                {
                    ...
                    if (pxTimer->uxAutoReload == (UBaseType_t) pdTRUE)
                    {
                        ...
                        xResult = newTimerGenericCommand(pxTimer, tmrCOMMAND_START_DONT_TRACE,
...
static void newProcessExpiredTimer(const TickType_t xNextExpireTime, const TickType_t xTimeNow)
{
    ...
    if (pxTimer->uxAutoReload == (UBaseType_t) pdTRUE)
    {
        if (newInsertTimerInActiveList(pxTimer, (xNextExpireTime + pxTimer->xTimerPeriodInTicks), xTimeNow,
                                       xNextExpireTime) == pdTRUE)
        {
            xResult = newTimerGenericCommand(pxTimer, tmrCOMMAND_START_DONT_TRACE, xNextExpireTime);
...
static void newSwitchTimerLists(void)
{
    ...
    while (listLIST_IS_EMPTY(newCurrentTimerList) == pdFALSE)
    {
        ...
        if (pxTimer->uxAutoReload == (UBaseType_t) pdTRUE)
        {
            ...
            if (xReloadTime > MAX_RTC_Counter)
            {
                xReloadTime = xReloadTime - MAX_RTC_Counter;
            }
