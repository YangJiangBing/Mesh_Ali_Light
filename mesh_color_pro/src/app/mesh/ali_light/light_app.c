...
{
	//DBG_DIRECT("===app handle dev state evt");
...
{
	//DBG_DIRECT("===app handle connect state evt"); //tianmao no le link its adv
...
T_APP_RESULT app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
    ...
    {
    	//DBG_DIRECT("===dis_server_id");
        app_result = app_dis_srv_cb((T_DIS_CALLBACK_DATA *)p_data);
    }
	else if(service_id == ais_server_id)
	{
		//DBG_DIRECT("===ais_server_id");	
	}
#endif
    return app_result;
}
...
bool prov_cb(prov_cb_type_t cb_type, prov_cb_data_t cb_data)
{
	...
	//DBG_DIRECT("===provisioning callback type = %d",cb_type);  //provisioning process 8-1-0-6-5-2-0
    switch (cb_type)
    {
    case PROV_CB_TYPE_PB_ADV_LINK_STATE:
        ...
        case PB_GENERIC_CB_LINK_OPENED:
			//DBG_DIRECT("===provisioning callback open link");
            if (mesh_node.node_state == UNPROV_DEVICE)
            {
        ...
        case PB_GENERIC_CB_LINK_CLOSED:
			//DBG_DIRECT("===provisioning callback close link");
            if (mesh_node.node_state == UNPROV_DEVICE)
            {
