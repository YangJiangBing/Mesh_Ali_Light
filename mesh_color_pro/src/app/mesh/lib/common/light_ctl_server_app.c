                                          ...
                                          void *pargs)
{
    ...
    case GENERIC_ON_OFF_SERVER_SET:
        {
						DBG_DIRECT("===light_ctl_server_app light_cw_turn_on_off");
...
static int32_t light_lightness_server_data(const mesh_model_info_p pmodel_info, uint32_t type,
                                           void *pargs)
{
	  DBG_DIRECT("===light_ctl_sever_app.c light_lightness_server_data");
    ...
    case LIGHT_LIGHTNESS_SERVER_SET:
        {
					  //DBG_DIRECT("===light_ctl_sever_app.c LIGHT_LIGHTNESS_SERVER_SET");
            ...
            if (pdata->total_time.num_steps == pdata->remaining_time.num_steps)
            {
				...
				//DBG_DIRECT("===light_ctl_sever_app.c  ctl.temperature :%d", ctl.temperature);
                light_set_ctl(ctl);
                light_state_store();
            }
        }
    ...
    case LIGHT_LIGHTNESS_SERVER_SET_LINEAR:
        {
					  //DBG_DIRECT("===light_ctl_sever_app.c LIGHT_LIGHTNESS_SERVER_SET_LINEAR");
            light_lightness_server_set_t *pdata = pargs;
            if (pdata->total_time.num_steps == pdata->remaining_time.num_steps)
            {
                light_ctl_t ctl = light_get_ctl();
...
static int32_t light_ctl_server_data(const mesh_model_info_p pmodel_info, uint32_t type,
                                     void *pargs)
{
    DBG_DIRECT("===light_ctl_sever_app.c light_ctl_server_data");
    switch (type)
    {
    ...
    case LIGHT_CTL_SERVER_SET:
        {
			//DBG_DIRECT("===enter LIGHT_CTL_SERVER_SET");
    ...
    case LIGHT_CTL_SERVER_SET_TEMPERATURE:
        {
			//DBG_DIRECT("===enter LIGHT_CTL_SERVER_SET_TEMPERATURE");
...
static int32_t scene_server_data(const mesh_model_info_p pmodel_info, uint32_t type,
                                 void *pargs)
{
	  DBG_DIRECT("===light_ctl_sever_app.c scene_server_data");
    switch (type)
    {
...
void light_ctl_server_models_init(uint8_t element_index)
{
	...
	//control light lightness server register
