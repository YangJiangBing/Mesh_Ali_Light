...
void light_prov_link_open(void)
{
   ...
   //LightCwT.cold = 65535;
   //LightCwT.warm = 65535;
   //light_set_cw_lightness(LightCwT);
...
void light_prov_link_closed(void)
{
    ...
    //light_blink_warm(1, 2, 50);
}

void light_prov_start(void)
{
	//DBG_DIRECT("===light prov start");
#if MESH_ALI_CERTIFICATION
#else
    //light_blink_red(2, 1, 50);
    //light_blink_warm(6, 6, 50); 
    turn_on_light();
#endif
}
...
void light_prov_complete(void)
{
    ...
    light_t *light = light_get_warm();
    light_blink(light,0,65535,500,50,6,light_prov_complete_cb);
