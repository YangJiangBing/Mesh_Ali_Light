...
static uint16_t light_lightness_process(const mesh_model_info_p pmodel_info,
                                        ...
                                        generic_transition_time_t trans_time)
{
    //DBG_DIRECT("===light lightness process");
...
#if MODEL_ENABLE_USER_STOP_TRANSITION_NOTIFICATION  //0
...
static int32_t light_lightness_delay_execution(mesh_model_info_t *pmodel_info, uint32_t delay_type)
{
    ...
    if (DELAY_EXECUTION_TYPE_LIGHT_LIGHTNESS == delay_type)
    {
        ...
        light_lightness_process(pmodel_info, plightness_info->target_lightness,
                                FALSE, plightness_info->trans_time);
    }
    else
    {
        plightness_info->delay_time_linear = 0;
        light_lightness_process(pmodel_info, plightness_info->target_lightness_linear,
                                TRUE, plightness_info->trans_time_linear);
    }
...
{
	//DBG_DIRECT("===light lightness server receive");
    bool ret = TRUE;
    ...
    case MESH_MSG_LIGHT_LIGHTNESS_SET_UNACK:
		//DBG_DIRECT("===light lightness pmesh_msg->access_opcode:%x",pmesh_msg->access_opcode);
        {
			//lightness struct
...
#if MODEL_ENABLE_DELAY_EXECUTION    //MODEL_ENABLE_DELAY_EXECUTION 0
            uint32_t delay_time = 0;
#endif
			//MEMBER_OFFSET(struct_t,member)宏的作用就是获得成员member在类型struct_t中的偏移量
			//判断接收的数据长度和实际是否一样
            if (pmesh_msg->msg_len == MEMBER_OFFSET(light_lightness_set_t, trans_time)) //
            {
            	//pmodel_info->indicate which model is sending/receiving this msg
                if (NULL != pmodel_info->model_data_cb)
                {
                	//save model info
            ...
            if (IS_GENERIC_TRANSITION_STEPS_VALID(trans_time.num_steps))
            {
				...
				//assert current lightness is range
