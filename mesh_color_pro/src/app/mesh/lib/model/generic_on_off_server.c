...
typedef struct
{
    ...
    uint32_t delay_time;
#endif
} generic_on_off_info_t, *generic_on_off_info_p;
...
static generic_on_off_t generic_on_off_process(const mesh_model_info_p pmodel_info,
                                               ...
                                               generic_transition_time_t trans_time)
{
    //DBG_DIRECT("===generic_on_off_process");
...
static int32_t generic_on_off_delay_execution(mesh_model_info_t *pmodel_info, uint32_t delay_type)
{
    ...
    generic_on_off_process(pmodel_info, ponoff_info->target_on_off, ponoff_info->trans_time);
...
{
    //DBG_DIRECT("======generic_on_off_server_receive");
    bool ret = TRUE;
    uint8_t *pbuffer = pmesh_msg->pbuffer + pmesh_msg->msg_offset;
    mesh_model_info_p pmodel_info = pmesh_msg->pmodel_info;
	
	//DBG_DIRECT("======access_opcode:%x",pmesh_msg->access_opcode);
