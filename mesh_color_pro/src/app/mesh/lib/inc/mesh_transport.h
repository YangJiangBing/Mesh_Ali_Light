...
typedef struct
{
    ...
    mesh_msg_t seg_msg; //!< save origing seg msg info
    uint32_t seq_zero; //!< save origin seq info
...
typedef struct
{
    ...
    uint16_t len; //!< length of seg msg
    mesh_msg_t seg_msg; //!< save origing seg msg info
    uint32_t seq_zero; //!< save origin seq info //only used 13 bits
...
void trans_handle_timeout(uint8_t type);
